...
> **Trạng thái:** FLF-171 (plan mode 1, P1 §5.6). **Bản chờ duyệt.** Khi nhóm duyệt xong thì đóng băng; sau đó mọi thay đổi phải qua PR nhãn `contract-change` và sửa các file zod bên dưới trong cùng PR.
...
| Ngày | PR | Thay đổi |
| --- | --- | --- |
| 2026-09-18 | FLF-171 (P1) | Bản đầu tiên. Đi kèm contract-change `Baseline.type` + `doc_version` trong `pipeline-contract.md` §3 |
