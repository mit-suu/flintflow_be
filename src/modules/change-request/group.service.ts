...
/** Gom vị trí có sửa/ghi chú thành group theo section của block; `not_related` không vào group. */
export const regroup = async (cr: IChangeRequest): Promise<void> => {
  ...
  const blocks = await DocBlock.find({ projectId: cr.projectId, doc_version: version!.version, block_id: { $in: locations.map((l) => l.block_id) } })
    .select("block_id section_id")
    .lean()
  const sectionOf = new Map(blocks.map((b) => [b.block_id, b.section_id ?? "misc"]))
  const bySection = new Map<string, IChangeLocation[]>()
  for (const l of locations) {
    if (l.conclusion !== "edit" && l.conclusion !== "comment") continue
    const key = sectionOf.get(l.block_id) ?? "misc"
